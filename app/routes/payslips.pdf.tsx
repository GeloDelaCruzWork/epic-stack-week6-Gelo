...
import { generatePDFFromHTML } from '#app/utils/pdf.server.ts'
import fs from 'fs'
...
export async function loader({ request }: LoaderFunctionArgs) {
	...
	// Find the payroll run for this pay period
			...
			try {
				const detail =
					typeof payslip.allowances_detail === 'string'
						? JSON.parse(payslip.allowances_detail as string)
	...
	try {
		...
		return new Response(pdf, {
			headers: {
				...
				'Content-Disposition': `inline; filename="payslips-${new Date().getTime()}.pdf"`,
