...
export function PayslipLayout({
	...
	useEffect(() => {
		if (showQRCode && data.verificationUrl) {
			QRCode.toDataURL(data.verificationUrl, {
				width: 80,
				margin: 0,
			}).then(setQrCodeDataUrl)
		}
	}, [showQRCode, data.verificationUrl])
				...
				<div className="payslip-section">
					...
					<table className="payslip-table">
						<tbody>
							...
							{data.earnings.allowances.map((allowance, index) => (
								<tr key={index}>
				...
				<div className="payslip-section">
					...
					<table className="payslip-table">
						<tbody>
							...
							{data.deductions.philhealthEE > 0 && (
								<tr>
									<td>PhilHealth</td>
									<td className="amount">
							...
							{data.deductions.withholdingTax > 0 && (
								<tr>
							...
							{data.deductions.loans.map((loan, index) => (
								<tr key={index}>
									<td>{loan.type}</td>
									<td className="amount">{formatCurrency(loan.amount)}</td>
								</tr>
							))}
							{data.deductions.otherDeductions.map((deduction, index) => (
								<tr key={index}>
